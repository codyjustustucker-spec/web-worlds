...
(() => {
  ...
  const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));
  let px=0,bx=0,by=0,bvx=190,bvy=-235,last=performance.now(),visible=true,complete=false,score=0,bricks=[],drag=false,pid=null,runStartedAt=performance.now();
  ...
  function resetGame(){score=0;complete=false;runStartedAt=performance.now();px=Math.max(0,(stage.clientWidth-paddle.offsetWidth)/2);if(scoreOut)scoreOut.textContent='0';if(status)status.textContent='PLAY';if(win)win.hidden=true;renderPaddle();layoutBricks(true);resetBall();last=performance.now();}
  ...
  function loop(t){
    ...
    if(bricks.length&&bricks.every(el=>el.classList.contains('is-hit'))){complete=true;bvx=bvy=0;if(status)status.textContent='CLEAR';if(win)win.hidden=false;const durationMs=Math.max(4000,Math.round(performance.now()-runStartedAt));window.dispatchEvent(new CustomEvent('webworlds:brick-breaker-complete',{detail:{mode:'brick-breaker',score,durationMs}}));}
...
(() => {
  ...
  if(breaker){
    ...
    let px=0,bx=0,by=0,bvx=135,bvy=-175,last=performance.now(),visible=true,complete=false,score=0,bricks=[],runStartedAt=performance.now();const rows=3,cols=5,gap=5;
    ...
    function resetGame(){complete=false;score=0;runStartedAt=performance.now();if(scoreOut)scoreOut.textContent='0';input.value='50';status.textContent='PLAY';updatePaddle();layoutBricks(true);resetBall();last=performance.now();}
